...
ws_client_t ws_connect_client(struct netconn* conn, char* url,
...
char* ws_hash_handshake(char* handshake, uint8_t len) {
	...
	memcpy(key, handshake, len);    			// Copy handshake vào key.
	strlcpy(&key[len], hash, sizeof(key));    // Nối hash vào đuôi handshake.
	mbedtls_sha1((unsigned char*)key, len + hash_len - 1, sha1sum); // Thực hiện băm sha-1.
	mbedtls_base64_encode(NULL, 0, &ret_len, sha1sum, 20);
	if(!mbedtls_base64_encode((unsigned char*)ret, 32, &ret_len, sha1sum, 20)) { // Mã hóa về định dạng base64.
		ret[ret_len] = '\0';
		return ret;
	}
	free(ret);
